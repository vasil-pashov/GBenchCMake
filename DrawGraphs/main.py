...
def gatherPlotDataFromGBenchmark(gBenchmark, allPlots, date=None):
  """Read all benchmarks from loaded json object, generated by google benchmark

  :param gBenchmark: Python representation of the JSON generated by google benchmark
  :type gBenchmark: dict
  :param allPlots: Output variable, where gathered info will be added
  :type allPlots: dict
  """
  
  benchmarks = gBenchmark['benchmarks']
  if date is None:
    date = datetime.datetime.strptime(gBenchmark["context"]["date"], "%m/%d/%y %H:%M:%S")
    ...
    with open(fileName, "r") as file:
      ...
      if "info" in jsonData:
        ...
        for benchmarkTask in jsonData['benchmark_list']:
          gatherPlotDataFromGBenchmark(benchmarkTask, allPlots, date=date)
      else:
        gatherPlotDataFromGBenchmark(jsonData, allPlots)
  return allPlots
